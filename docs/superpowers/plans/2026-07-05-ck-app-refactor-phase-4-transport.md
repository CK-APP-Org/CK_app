# CK App Refactor — Phase 4-A (TransportPage.vue Split) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task (inline execution — this repo's current preference, not subagent-driven-development). Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split `src/pages/TransportPage.vue` (1742 lines) into a thin orchestrator plus two self-contained section components (`YoubikeSection.vue`, `MetroSection.vue`) and one pure-logic composable module (`useYoubike.js`), with zero behavior change.

**Architecture:** `TransportPage.vue` keeps only the FAB (`showMenu`/`toggleMenu`) and renders `<youbike-section>`/`<metro-section>`. Each section owns its own state, Vuex access, dialogs, and — per `docs/refactoring-plan.md` Phase 4-A — its own independent 10s polling interval (the original file ran one combined `setInterval` for both YouBike and Metro; this splits it into two, since they hit different, unrelated APIs). The FAB's two "add station" buttons live in the parent but must open a dialog owned by a child — done via `defineExpose({ openAddStationDialog })` on each section and a template `ref` in the parent, the standard Vue 3 pattern for a parent invoking a child's internal state.

**Tech Stack:** Quasar 2 / Vue 3 `<script setup>`, Vuex 4, `@vue-leaflet/vue-leaflet`, axios, date-fns.
...
- This is **sub-phase 1 of 4** in Phase 4 (Transport → Todo → Settings → Food). Per the process decision made when starting Phase 4, all four sub-phases land as separate commits on ONE branch/PR (`refactor/phase-4-component-splitting`) — **do not open a PR at the end of this plan**; the PR opens once after the Food sub-phase.
- Pure code motion — no behavior change, no dead-code cleanup beyond what's noted explicitly below. Every function body below is copied verbatim from the current file (verified by direct read on 2026-07-05, current HEAD of this branch).
- New components use `<script setup>` (already the established style for the newest page in this codebase, `FoodPage.vue`; also the style Phase 4-B's plan will convert `TodoPage.vue` to) rather than the older `defineComponent({ setup() {...} })` style most other pages still use.
- All imports use relative paths (`../../store/index`, etc.), matching this codebase's existing convention (confirmed: no page currently imports via the `src/`/`components/` jsconfig aliases except one boot file).

---

## Noted deviations from a pure line-for-line move (call these out in the PR description)

1. **Dropped one dead import.** The original file has `import store from "../store/index";` at the top of `<script>`, immediately shadowed by `const store = useStore();` inside `setup()` — the top-level import is never referenced again anywhere in the file. It is not carried into either new component.
2. **Five CSS rules are dead code with no clear owner.** `.custom-card-margin`, `.update-time`/`.update-time .q-icon`, `.search-btn`, `.find-nearest-btn`, and `.container` don't match any class used in `TransportPage.vue`'s template (verified by grep). They're arbitrarily kept in `YoubikeSection.vue`'s `<style>` block, since dead CSS causes no behavior difference regardless of which file it lands in.
3. **A duplicate `.menu-btn` CSS rule is preserved as-is.** The original stylesheet defines `.menu-btn` twice (once at the top of the Metro block, once near the bottom) with different property values — the second silently wins by cascade order. Both copies move into `MetroSection.vue` unchanged; fixing the duplicate is out of scope for this phase (it's a pre-existing quirk, not something this split introduces).
4. **Polling is split into two independent intervals**, per `docs/refactoring-plan.md` Phase 4-A's explicit instruction ("Polling (`setInterval`) moves into each section independently — they fetch different APIs anyway").

---

## Task 1: Create `src/composables/useYoubike.js`

**Files:**
- Create: `src/composables/useYoubike.js`

**Interfaces:**
- Consumes: nothing.
- Produces (used by Task 2): `Station` class, `districtOptionsTPC`, `districtOptionsNTC` arrays, `calculateDistance(lat1, lon1, lat2, lon2)`, `deg2rad(deg)`, `parseTimestamp(timestamp)`, `timeAgo(time)` — all named exports, all pure (no Vue reactivity, no component context needed). `parseTimestamp`/`timeAgo`/`calculateDistance` are also Phase 7's future unit-test targets per `docs/refactoring-plan.md`.
...
```bash
node --input-type=module -e "$(cat src/composables/useYoubike.js); console.log(typeof Station, typeof calculateDistance)"
```
...
```bash
git add src/composables/useYoubike.js
git commit -m "refactor(transport): extract useYoubike composable

Pure, Vue-independent functions/data pulled out of TransportPage.vue
ahead of the component split: the Station class, the two district
option lists, calculateDistance/deg2rad, and parseTimestamp/timeAgo.
No behavior change — verbatim move."
```

---

## Task 2: Create `src/components/transport/YoubikeSection.vue`

**Files:**
- Create: `src/components/transport/YoubikeSection.vue`

**Interfaces:**
- Consumes: `Station`, `districtOptionsTPC`, `districtOptionsNTC`, `calculateDistance` from `../../composables/useYoubike` (Task 1).
- Produces (used by Task 4): renders itself with no props; exposes `openAddStationDialog()` via `defineExpose` for the parent to call.
...
```bash
git add src/components/transport/YoubikeSection.vue
git commit -m "refactor(transport): extract YoubikeSection.vue

All YouBike station-card grid markup, the 5 YouBike-only dialogs (add
station, edit nickname, delete station, nearest stations, location
picker), and their refs/methods, moved verbatim out of
TransportPage.vue. Polling now runs in this component's own
onMounted/onUnmounted (previously combined with Metro's polling in
one shared setInterval). Exposes openAddStationDialog() for the
parent's FAB to call.

TransportPage.vue is not updated to use this yet — that's Task 4,
after MetroSection.vue also exists — so the app will not build/run
correctly again until this branch's Task 4 commit lands."
```

---

## Task 3: Create `src/components/transport/MetroSection.vue`

**Files:**
- Create: `src/components/transport/MetroSection.vue`

**Interfaces:**
- Consumes: `metroLineColors`, `stationLines` from `../../data/metroData` (unchanged, pre-existing module).
- Produces (used by Task 4): renders itself with no props; exposes `openAddStationDialog()` via `defineExpose` for the parent to call.
...
```bash
git add src/components/transport/MetroSection.vue
git commit -m "refactor(transport): extract MetroSection.vue

All Metro station-list markup, the crowdedness legend, the add-metro-
station dialog, and their refs/methods, moved verbatim out of
TransportPage.vue. Polling now runs in this component's own
onMounted/onUnmounted (previously combined with YouBike's polling in
one shared setInterval). Exposes openAddStationDialog() for the
parent's FAB to call. Preserves the pre-existing duplicate .menu-btn
CSS rule as-is (both copies moved, not deduplicated — out of scope
for this phase).
...
## Task 4: Rewrite `src/pages/TransportPage.vue`

**Files:**
- Modify: `src/pages/TransportPage.vue` (1742 lines → ~35 lines)

**Interfaces:**
- Consumes: `YoubikeSection` (Task 2) and `MetroSection` (Task 3) as components; calls `.openAddStationDialog()` on each via template refs.
- Produces: nothing another task needs — this is the last file in the Transport split.
...
<template>
  <q-page class="bg-grey-1 q-pa-sm">
    ...
    <metro-section ref="metroSectionRef" />
...
<script setup>
import { ref } from "vue";
import YoubikeSection from "../components/transport/YoubikeSection.vue";
import MetroSection from "../components/transport/MetroSection.vue";

const showMenu = ref(false);
const youbikeSectionRef = ref(null);
const metroSectionRef = ref(null);
...
const openAddMetroStationDialog = () => {
  metroSectionRef.value.openAddStationDialog();
  showMenu.value = false;
};
</script>
...
```bash
git add src/pages/TransportPage.vue
git commit -m "refactor(transport): wire TransportPage.vue to the two new sections

TransportPage.vue drops from 1742 to ~35 lines. It now only owns the
FAB (showMenu/toggleMenu) and renders YoubikeSection/MetroSection,
calling openAddStationDialog() on whichever section's FAB button was
clicked via a template ref. This completes the Transport split
(Phase 4-A of docs/refactoring-plan.md)."
```
...
- [ ] **Step 1: Confirm no leftover references to moved-away names in TransportPage.vue**

```bash
grep -n "Station\b\|stations\b\|metroStations\b\|fetchYoubikeData\|fetchTrackInfo" src/pages/TransportPage.vue
```
Expected: no output (every one of these now lives only in the two new section components).
...
yarn lint
```
Expected: exit 0. If it fails, read the message — most likely cause is a missed import in one of the two new `<script setup>` files (e.g. forgetting `Icon` or `useQuasar`).
...
yarn dev
```
Via browser automation, navigate to `/#/transport` and confirm:
- No console errors on load.
- YouBike station cards render with rent/return counts (or a loading spinner initially).
- Metro station sections render with arrival countdowns (or a loading spinner initially).
- Tap the FAB (bottom-right "+"): both "YouBike" and "北捷" menu items appear; tapping "YouBike" opens the add-YouBike-station dialog, tapping "北捷" opens the add-metro-station dialog (each opened from the SAME FAB in the parent, dispatching into the correct child — this is the one behavior this refactor could plausibly break, so it's the most important thing to check).
- Edit a YouBike station's nickname; delete a YouBike station; delete a Metro station via its own menu — all three still work.
- Wait ~10s and confirm both YouBike counts and Metro countdowns update on their own (independent polling intervals both still firing).

- [ ] **Step 4: If everything passes, no additional commit needed here** — Tasks 1-4 already committed their own changes.
...
**Files:**
- Create: `docs/decisions/phase-4-component-splitting/task-1-transport.md`

Follow the established pattern (`docs/decisions/phase-3-remove-login/`): "What changed, in plain language" → "Why this is safe / why this decision was made" → a concept explainer aimed at someone new to this codebase (this one's natural subject: what `defineExpose`/template refs are and why the FAB needs them to reach into a child component's dialog state, using this task's actual `openAddStationDialog()` pattern as the example) → "Verification performed" (mirroring Task 5's steps above).

- [ ] **Step 1: Write the doc** (content per the pattern above — not reproduced here since it's freeform prose, not code the plan can pre-specify placeholder-free)

- [ ] **Step 2: Commit**

```bash
git add docs/decisions/phase-4-component-splitting/task-1-transport.md
git commit -m "docs: add beginner-friendly Task 1 decision doc for Phase 4 (Transport)"
```
...
- **Spec coverage:** `docs/refactoring-plan.md` Phase 4-A's three named deliverables (`YoubikeSection.vue`, `MetroSection.vue`, `src/composables/useYoubike.js`) are all covered; the spec's instruction that `TransportPage.vue` retain only the FAB and that polling move into each section independently are both implemented (Tasks 2-4).
- **Placeholder scan:** No TBD/TODO in any code step. Every new file's complete content is given (these are net-new files, so — unlike Phase 3's deletion-only plan — there was no boundary-anchoring option available; full content was required and given for all three new files).
- **Type/name consistency:** Cross-checked `openAddStationDialog` is named identically in both `YoubikeSection.vue`/`MetroSection.vue` (Tasks 2-3) and the parent's calls to `youbikeSectionRef.value.openAddStationDialog()` / `metroSectionRef.value.openAddStationDialog()` (Task 4). Cross-checked every import in Tasks 2-3 against the functions/constants actually used in that file's body (e.g. `MetroSection.vue` does NOT import `useQuasar` or any leaflet component, since `$q` and maps are YouBike-only).
- **Next sub-phase:** Phase 4-B (`TodoPage.vue`) gets its own plan document, written after this one is executed and committed — per the writing-plans skill's guidance to treat independent subsystems as separate plans, and to avoid drafting Todo/Settings/Food content that could go stale before Transport's actual extraction reveals any surprises.
